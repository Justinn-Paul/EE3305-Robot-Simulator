/*
  EE3305/ME3243
  Name: Batungbacal Justin Paul Francisco
  Matric number: A0239124J
*/
...
std::vector<std::array<int, 2>> findPath(int idx_x, int idx_y, int goal_x, int goal_y)
{
  ...
  while (!queue.empty())
  {
    ...
    // ENTER YOUR COMMENT HERE (GRADED)
    Node *cur_node = queue.front(); //  initializing the current front node of the queue as the current node (or reference)
    queue.pop_front();              //  take out the front node (cur_node) from the queue, since queues uses FIFO
    int cur_x = cur_node->x;        // the assignments of the coordinates and the cost of cur_node for the if statement below
    int cur_y = cur_node->y;
    int cur_cost = cur_node->cost;

    // ---------------------- (2) Find the path if goal is found, and break loop -------------------------
    if (cur_x == goal_x && cur_y == goal_y)   // checking the condition if the current cell (cur_node) is the target cell
    {                                         // if the cell is the target cell, then complete the path and exit the while loop
      Node *node = cur_node;
      do
      {
        path.push_back({node->x, node->y}); // place the target cell to the back of the queue, last cell visited in the path
        node = node->parent;                // traverse to the parent (previous cell), working backwards from the end of queue
      } while (node != nullptr);            // checks if the node is not pointing to a null location in the memory address, 
      break;                                // once there are no more nodes in the queue break the do while loop
    }                                       // the do while loop essentially reverses the queue with the target cell at the front
